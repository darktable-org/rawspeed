...
void PefDecoder::decodeMetaDataInternal(const CameraMetaData* meta) {
  ...
  if (mRootIFD->hasEntryRecursive(static_cast<TiffTag>(0x0201))) {
    ...
    if (wb->count == 4) {
      mRaw->metadata.wbCoeffs[0] = lossless_cast<float>(wb->getU32(0));
      mRaw->metadata.wbCoeffs[1] = lossless_cast<float>(wb->getU32(1));
      mRaw->metadata.wbCoeffs[2] = lossless_cast<float>(wb->getU32(3));
    }
  }
}
